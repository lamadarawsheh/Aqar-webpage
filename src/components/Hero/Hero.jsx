...
const Hero = () => {
  const { ref: heroRef, isVisible: heroVisible } = useFadeIn({ threshold: 0.1 });
  const { ref: socialsRef, isVisible: socialsVisible } = useFadeIn({ threshold: 0.1 });
  ...
  return (
    <section className="relative w-full overflow-hidden bg-[#0b1928]">
      <img
        ref={heroRef}
        src="/hero-bg.jpg"
        alt="Dream and develop for the future"
        className={`block w-full h-auto z-[1] ${heroVisible ? 'hero-animate' : 'hero-hidden'}`}
      />

      <div
        ref={socialsRef}
        className={`absolute bottom-[40px] left-[40px] flex gap-[10px] z-10 fade-hidden fade-up ${socialsVisible ? 'fade-visible' : ''}`}
        style={{ direction: 'ltr', transitionDelay: '0.4s' }}
      >
